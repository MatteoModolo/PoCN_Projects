...
# comparison loop, the second month is kept for the next iteration
for (i in 2:length(network_files)) {
  ...
  # checking persistent, appearing and disappearing nodes
  ...
  # checking persistent, appearing and disappearing edges
...
# Reshape data for plotting NODES
plot_data <- churn_data %>%
  select(month2, nodes_persistent, nodes_appearing, nodes_disappearing) %>%
  # Make disappearing negative so it plots downwards
  mutate(nodes_disappearing = -nodes_disappearing) %>%
  pivot_longer(cols = c(nodes_persistent, nodes_appearing, nodes_disappearing), 
...
c <- ggplot(plot_data, aes(x = month2, y = Count, fill = Status)) +
  ...
  # Map the correct node statuses to the colors
  scale_fill_manual(values = c("nodes_appearing" = "blue", 
                               "nodes_persistent" = "grey70", 
                               "nodes_disappearing" = "red")) +
  scale_x_discrete(breaks = c(start_month, end_month)) +
  # Add comma formatting for large node numbers (optional but looks clean)
  scale_y_continuous(labels = scales::comma) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5, face = "bold", size = 11)) +
  labs(title = "Month-over-Month BGP Node Turnover",
       x = "Timeframe",
       y = "Number of Nodes (ASNs)",
       fill = "Node Status")
